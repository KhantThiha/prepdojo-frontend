...
export const PromptInputAttachments = ({ children }: PromptInputAttachmentsProps) => {
  ...
  return files.map((file) => <Fragment key={file.id}>{children(file)}</Fragment>);
};
...
export const PromptInputAttachment = ({ data }: PromptInputAttachmentProps) => {
  ...
  return (
    <div className="bg-muted text-muted-foreground flex items-center gap-2 rounded-md px-2 py-1 text-xs">
      <span className="max-w-24 truncate">{data.filename}</span>
      <button
